...
last_updated = '30th November, 2021'
...
def _buildOutputsToUnitMap():
	...
	print(' -- [FOON] : Building output-to-FU dictionaries..')
...
def _buildObjectToUnitMap():
	...
	print(' -- [FOON] : Building object-to-FU dictionaries..')
...
def _buildFunctionalUnitMap():
	...
	print(' -- [FOON] : Building FU-to-FU dictionaries..')
...
def _addObjectToFOON(newObject, isInput, D, newFU_lvl3, newFU_lvl2, newFU_lvl1):
	...
	if isInput == True:
		...
		newFU_lvl3.addObjectNode(objectNode=nodes_lvl3[objectIndex], is_input=True, objectInMotion=int(D))
	else:
		# -- add the Objects as output nodes to the FU:
		newFU_lvl3.addObjectNode(objectNode=nodes_lvl3[objectIndex], is_input=False, objectInMotion=int(D))
	...
	if isInput == True:
		newFU_lvl2.addObjectNode(nodes_lvl2[objectIndex], is_input=True, objectInMotion=int(D))
	else:
		newFU_lvl2.addObjectNode(nodes_lvl2[objectIndex], is_input=False, objectInMotion=int(D))
	...
	if isInput == True:
		newFU_lvl1.addObjectNode(nodes_lvl1[objectIndex], is_input=True, objectInMotion=int(D))
	else:
		newFU_lvl1.addObjectNode(nodes_lvl1[objectIndex], is_input=False, objectInMotion=int(D))
...
def _performExpansion():
	...
	global FOON_functionalUnits, FOON_nodes, FOON_outputsToUnits, FOON_objectsToUnits, FOON_functionalUnitMap, FOON_labels, FOON_objectSenses
	...
	_response = input(' -- [FOON-EXP] : Expand using 1) text-based method or 2) non-text-based method? [1/2] (default: 1) > ')
